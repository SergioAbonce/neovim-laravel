return {
  setup = function(ops)
    ops = vim.tbl_deep_extend("force", require "plugins.config.lsp.default", ops)
    ...
    for server, options in pairs(ops.servers) do
      if options.enable == true then
        local ok, definition = pcall(require, "plugins.config.lsp.servers." .. server)
    ...
    -- can get the client by name of the files

    vim.api.nvim_create_user_command("AllLSP", function()
      vim.ui.select(servers, { prompt = "LSP Servers" }, function(server)
        ...
        local path = string.format("%s/lua/plugins/config/lsp/servers/%s.lua", vim.fn.stdpath "config", server)
